		...
		while ( false !== ($file = readdir ( $handle )) ) {
			...
			if ($file != "." && $file != ".."&&strpos($file,".")) {
				...
				if(strpos($file,'.php')!==false && $file!='index.php'){
					...
					if(strpos($data,"define('APP_HOME','Admin')")!==false){
						$admin_url = $file;
						break;
					}
...
switch($act){
	...
	case 'step3':
		...
		$tpl = include('tpl/step3.php');
	break;
	case 'step4':
		$tpl = include('tpl/step4.php');
	break;
	case 'step5':
		$admin_url = get_domain().'/'.get_admin_url();
		//生成安装锁定文件
		$res = file_put_contents('install.lock','');
		$tpl = include('tpl/step5.php');
	break;
	case 'install_testdb':
		...
		$sql = str_replace('k_',$config['db']['prefix'],$sql);
		$count=100;
		$sql = substr($sql,14);
		$sql.="UPDATE `k_level` SET `name`='".$_POST['admin_name']."',`pass`='".md5(md5($_POST['admin_pass']).'YF')."' , `regtime` = '".time()."' , `logintime` = ".time()."   WHERE id=1";
