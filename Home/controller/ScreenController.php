...
class ScreenController extends CommonController
{
	function index(){
		...
		//检测三个参数是否存在
		if(!$this->frparam('molds',1) || !$this->frparam('tid') || !$this->frparam('k_screen',1)){
			$this->error('参数错误！');
		}
		...
		//检测是否有page分页参数
		$this->frpage = 1;
		if(strpos($this->frparam('k_screen',1),'page')!==false){
			$k_screen_arr = explode('-page-',$this->frparam('k_screen',1));
			$k_screen = explode('-',$k_screen_arr[0]);
			$this->frpage = (int)$k_screen_arr[1];
		}else{
			$k_screen = explode('-',$this->frparam('k_screen',1));
		}
		...
		$screen = array_combine($k_screen_key,$k_screen_value);
