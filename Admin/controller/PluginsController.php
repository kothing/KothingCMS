...
class PluginsController extends CommonController
{
	...
	function output(){
		...
		if ($zip->open($filepath.'.zip', \ZipArchive::CREATE|\ZipArchive::OVERWRITE) === TRUE) {
			$this->addFileToZip(APP_PATH.'Admin/exts/'.$filepath.'/', $zip); //调用方法，对要打包的根目录进行操作，并将ZipArchive的对象传递给方法
	...
	function action_do(){
		...
		if($filepath){
			if($type==1){
				...
				$plg = new \Admin\exts\PluginsController($this->frparam());
				...
				if(!$plugins){
					if($type==2){
						//批量删除文件
						if(is_dir(APP_PATH.'Admin/exts/'.$filepath)){
							deldir(APP_PATH.'Admin/exts/'.$filepath);
						}
	...
	function update(){
		...
		if($filepath){
			if($this->frparam('action',1)){
				...
				switch ($action) {
				    ...
				    case 'file-upzip':
						...
						$path = APP_PATH.'Admin/exts/';
						$msg = $this->get_zip_originalsize($tmp_path,$path);
						if($filepath=='kothingUpdate'){
							$isinstall = true;
						}else{
			...
			$config = require_once(APP_PATH.'Admin/exts/'.$filepath.'/config.php');
